...
export async function getFile(imageUrl: string, res: Response): Promise<void> {
  ...
  try {
    ...
    const contentTypeStr = (Array.isArray(rawContentType) ? rawContentType[0] : String(rawContentType ?? "")) || "application/octet-stream";
    res.header("Content-Type", contentTypeStr);
    res.type(contentTypeStr.split(";")[0]);
